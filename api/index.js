...
console.log("--- API index.js loaded ---"); // デバッグログ1
...
app.post("/api/evaluate-comments", async (req, res) => {
    try {
        ...
        const prompt = `
        あなたは経験豊富なシニアソフトウェアエンジニアとして、初心者がC言語のプログラムに書いたコードコメントをレビューしてください。
        以下の6つの評価基準に基づき、コメントの品質を100点満点で採点し、各項目について具体的なフィードバックを日本語で提供してください。
        プログラムはAIが自動生成したものであり、そのプログラムに対してジュニア開発者がコメントを書き加えたものです。具体的なプログラムの用途や意味はわかりません。
        そのため、以下のルールを必ず守ってください。
        - 最重要：プログラム中にコメントが全くない場合は、全ての評価基準で0点とし、フィードバックも「コメントが全くないため評価できません。」とだけ答えてください。
        - プログラム自体の評価は行わず、あくまでコメントの品質のみを評価してください。
        - プログラム自体はAIが自動生成したものなので、「このプログラムがどのような意図で作られたのか」を補足するコメントを初心者は書くことはできません。その点は評価に含めないでください。
