import { getHighScore, saveHighScore, getTopMistakeKeys, saveTopMistakeKeys, saveUserTypingSpeed, addTypingSession, updateAverageSpeedIfNeeded, updateDailyAverageSpeed, saveUserProgram, getUserPrograms, toggleFavoriteProgram, getProgramMessages, addProgramMessage, deleteProgramMessage, getRecentPrograms, addRecentProgram } from './firebase_helper.js';
...
async function endGame(wasStoppedManually) {
    ...
    if (wasStoppedManually) {
        ...
        // Save the completed program to history
        if (codeLines.length > 0) {
            await addRecentProgram(user, currentDifficulty, codeLines.join('\n'));
        }
...
async function fetchWords(customTheme = "", topMistakeKeys = []) {
    try {
        ...
        let recentPrograms = [];
        if (user) {
            recentPrograms = await getRecentPrograms(user, currentDifficulty);
        }
